...
class MultiNestSampler(SourceData):
    ...
    def compute_bayes_factor(self, model: str, seed=np.random.get_state()[1][0],):
        ...
        file_path = f'/discover/nobackup/cchirent/multinest/chains/{detector_label}_{self.q_mass}_{label_data}_{label_model}_mass_{round(mass,1)}_redshift_{self.redshift}_seed_{seed}/'
    ...
    def compute_parameters_multi_modes(
        ...
        seed = np.random.get_state()[1][0]
        file_path = f'/discover/nobackup/cchirent/multinest/chains/pars_{detector_label}_{self.q_mass}_{label_data}_{label_model}_mass_{round(mass,1)}_redshift_{self.redshift}_seed_{seed}/'
        pathlib.Path(file_path).mkdir(parents=True, exist_ok=True)
...
def compute_logB_2modes(q, sub_mode, detector, mass, redshifts, cores=4):
...
if __name__ == '__main__':
    ...
    masses = np.logspace(np.log10(10), np.log10(5e4), N_masses, endpoint=True)
    ...
    for mode in modes:
        ...
        compute_logB_2modes(mass_ratio, mode, detector, mass, redshifts, cores)
